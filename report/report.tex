...
Dans ce chapitre, nous présentons le modèle binomial de Cox, Ross et Rubinstein, souvent appelé modèle CRR. Ce modèle repose sur une représentation discrète de l'évolution du prix du sous-jacent. À chaque période, le prix peut seulement évoluer selon deux possibilités : une hausse ou une baisse. Cette structure simple permet de construire un arbre de prix et de calculer progressivement la valeur de l'option.

\section{Principe général du modèle binomial}

Le modèle de Cox, Ross et Rubinstein a été introduit en 1979. Il propose une méthode de valorisation des options fondée sur une représentation discrète du temps. Au lieu de supposer que le prix du sous-jacent évolue continûment, le modèle découpe la période de vie de l'option en un nombre fini d'étapes.
...
Dans le modèle CRR, si le prix du sous-jacent vaut $S$ à une date donnée, alors à la période suivante il peut devenir soit
\[
S u,
\]
en cas de hausse, soit
\[
S d,
\]
en cas de baisse.
...
Comme tout modèle financier, le modèle CRR repose sur plusieurs hypothèses simplificatrices. Ces hypothèses permettent d'obtenir un cadre mathématique clair pour valoriser les options.

\begin{definition}{Marché sans friction}{marche-sans-friction}
Un marché sans friction est un marché dans lequel on suppose qu'il n'existe pas de coûts de transaction, pas de taxes, et que les actifs peuvent être achetés ou vendus librement.
\end{definition}

Dans le cadre du modèle CRR, on suppose d'abord que le marché est sans friction. Cette hypothèse simplifie les calculs, car elle permet de ne pas tenir compte des frais liés aux achats et ventes d'actifs.

On suppose également que le taux sans risque reste constant pendant toute la durée de vie de l'option. Pour éviter toute confusion avec les rendements du sous-jacent, on note ce taux $r_f$.

\begin{definition}{Taux sans risque constant}{taux-sans-risque-constant}
Le taux sans risque est supposé constant lorsque sa valeur ne change pas pendant toute la période étudiée.

Dans le modèle CRR, ce taux est noté $r_f$.
\end{definition}

Le modèle suppose aussi que la volatilité du sous-jacent est constante. Cette hypothèse signifie que l'intensité des variations du prix de l'actif ne change pas au cours du temps.

\begin{definition}{Volatilité constante}{volatilite-constante}
La volatilité est dite constante lorsque le paramètre $\sigma$ reste identique pendant toute la durée de vie de l'option.
\end{definition}

Enfin, le modèle repose sur l'absence d'opportunité d'arbitrage. Cette hypothèse est fondamentale, car elle garantit qu'il n'est pas possible de réaliser un profit certain sans prendre de risque.
...
Ces hypothèses ne décrivent pas parfaitement la réalité des marchés financiers. Cependant, elles permettent de construire un modèle simple, cohérent et utilisable pour comprendre les mécanismes de valorisation des options.
...
Cette relation permet d'obtenir un arbre recombiné. Cela signifie qu'une hausse suivie d'une baisse conduit au même prix qu'une baisse suivie d'une hausse :
\[
S_0 u d = S_0 d u.
\]

Cette propriété est importante, car elle réduit fortement le nombre de nœuds à calculer dans l'arbre.

Le modèle CRR repose donc sur une construction discrète du prix du sous-jacent à partir de paramètres précis, notamment le pas de temps $\Delta t$, les facteurs de hausse et de baisse $u$ et $d$, ainsi que la probabilité risque-neutre $p^*$. Ces paramètres ne sont pas choisis arbitrairement : ils doivent être déterminés à partir des caractéristiques de l'option et des données de marché disponibles.

Le chapitre suivant est consacré à cette étape de calibration. Il présente la méthode utilisée pour récupérer les données de marché, nettoyer les séries de prix, calculer les rendements logarithmiques, estimer la volatilité historique et déterminer les paramètres nécessaires à la mise en œuvre du modèle CRR.
% ============================================================
% CHAPITRE 3
% ============================================================
