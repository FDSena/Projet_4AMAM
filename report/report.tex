...
Cette référence théorique est justifiée par le résultat de convergence
rappelé dans ce même chapitre : lorsque le nombre d'étapes $N$ tend vers
l'infini, le prix CRR converge vers le prix de Black-Scholes~\cite{crr1979}.
Comparer CRR à Black-Scholes revient donc à mesurer la qualité de
...
chapitre~3), puis utilisée à la fois dans le modèle CRR et dans la
formule de Black-Scholes rappelée ci-dessous, de sorte que les deux
prix soient bien comparables.

Pour un call européen, le prix de Black-Scholes est (voir
section~\ref{sec:lien-black-scholes}) :
\[
C_{\mathrm{BS}} = S_0\,\Phi(d_1) - K\,e^{-rT}\,\Phi(d_2),
\]
avec
...
réduite. Cette formule est évaluée avec les mêmes paramètres $S_0$,
$K$, $T$, $r$ et $\sigma$ que ceux utilisés pour construire l'arbre CRR.
...
Soient $\hat{V}_i$ le prix prédit par CRR et $V_i^{\mathrm{BS}}$ le prix
de référence Black-Scholes pour l'actif $i$, avec $i = 1, \ldots, n$.
L'\textit{erreur absolue moyenne} est définie par :
\[
\mathrm{MAE} = \frac{1}{n} \sum_{i=1}^{n} \bigl|\hat{V}_i - V_i^{\mathrm{BS}}\bigr|.
\]
...
La \textit{racine de l'erreur quadratique moyenne} est définie par :
\[
...
et Black-Scholes est bien une propriété systématique liée à la
discrétisation, indépendante de l'actif considéré.
...
étapes approxime le prix de Black-Scholes avec une erreur relative de
l'ordre de $0{,}18\,\%$ sur tous les actifs testés, quels que soient le
niveau de volatilité ou le prix absolu du sous-jacent. Cette performance
...
valeur Black-Scholes en présentant des oscillations de plus en plus
amorties à mesure que $N$ augmente, comme le confirme l'étude de
convergence ci-dessous~\cite{hull2018}.
...
\caption{Convergence du prix CRR vers Black-Scholes en fonction de $N$
         (TotalEnergies, call ATM, $T=1$~an, $r=3\,\%$,
         $\sigma \approx 0{,}253$).}
...
$N \geq 50$. Ce comportement est cohérent avec les observations du
chapitre~5 sur l'influence du nombre d'étapes.
...
formulations standards, sont discutés plus en détail dans le
chapitre~7~\cite{hull2018}.
