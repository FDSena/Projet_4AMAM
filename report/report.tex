...
Ce rapport porte sur la valorisation d'\textbf{options financières} à l'aide du \textbf{modèle binomial} de \textbf{Cox-Ross-Rubinstein}. Ce modèle permet de représenter l'évolution possible du prix d'un actif sous-jacent à l'aide d'un \textbf{arbre binomial}, dans lequel le prix peut augmenter ou diminuer à chaque étape.

Après avoir présenté les notions fondamentales liées aux options financières, le rapport introduit le fonctionnement du modèle CRR et les paramètres nécessaires à son application. Une attention particulière est portée à la \textbf{calibration} de ces paramètres à partir de données de marché, notamment à travers le calcul des rendements logarithmiques, l'estimation de la \textbf{volatilité historique} et le choix du \textbf{taux sans risque}.

L'étude est appliquée à différents \textbf{sous-jacents}, le CAC 40 et le Brent, afin d'observer le comportement du modèle dans plusieurs contextes de marché. Le modèle est ensuite implémenté informatiquement pour calculer le prix d'\textbf{options européennes} et \textbf{américaines}, puis analyser l'influence des principaux paramètres sur les résultats obtenus.
...
\textbf{Mots-clefs:} Option financière, modèle binomial, Cox-Ross-Rubinstein (CRR), arbre binomial, calibration, volatilité historique, taux sans risque, options européennes, options américaines, sous-jacent.
...
This report focuses on the valuation of \textbf{financial options} using the \textbf{Cox-Ross-Rubinstein (CRR)} \textbf{binomial model}. This model represents the possible evolution of an \textbf{underlying asset} price through a \textbf{binomial tree}, where the price can either increase or decrease at each time step.

After introducing the main concepts related to financial options, the report presents the CRR model and the parameters required for its implementation. Particular attention is given to the \textbf{calibration} of these parameters from market data, through the computation of logarithmic returns, the estimation of \textbf{historical volatility} and the choice of a \textbf{risk-free rate}.

The study is applied to different \textbf{underlying assets}, including the CAC 40 and Brent, in order to observe the behaviour of the model in several market contexts. The model is then implemented computationally to price \textbf{European} and \textbf{American options} and to analyse the influence of the main parameters on the results obtained.
...
\textbf{Keywords:} Financial option, binomial model, Cox-Ross-Rubinstein (CRR), binomial tree, calibration, historical volatility, risk-free rate, European options, American options, underlying asset.
...
Les options financières occupent une place centrale dans les marchés financiers modernes. Elles permettent aux investisseurs de se couvrir contre des risques de marché, de spéculer sur l'évolution future d'un actif ou encore de construire des stratégies d'investissement élaborées. Leur valorisation constitue un enjeu fondamental, car le prix d'une option dépend de nombreux paramètres financiers et mathématiques dont l'interaction n'est pas toujours intuitive.

Parmi les modèles de valorisation existants, le modèle binomial de Cox, Ross et Rubinstein, introduit en 1979, occupe une place particulière. Il repose sur une représentation discrète et progressive de l'évolution du prix d'un actif sous-jacent, ce qui le rend à la fois simple à comprendre et suffisamment puissant pour traiter des options européennes et américaines. Sa structure en arbre binomial permet de visualiser concrètement les différents scénarios possibles et de remonter progressivement vers le prix de l'option à la date initiale.

L'objectif de ce rapport est de présenter de manière complète le modèle CRR, depuis ses fondements théoriques jusqu'à son implémentation informatique et sa validation empirique sur des données de marché réelles. Nous cherchons à répondre à la question suivante: dans quelle mesure le modèle CRR permet-il de valoriser de manière fiable des options financières à partir de données historiques ?

Pour cela, le rapport s'articule autour de sept chapitres. Le chapitre~1 pose le cadre théorique financier en introduisant les notions fondamentales liées aux options: actif sous-jacent, prix d'exercice, maturité, payoff, options européennes et américaines. Le chapitre~2 présente le modèle de Cox-Ross-Rubinstein, sa construction par arbre binomial, la probabilité risque-neutre et le lien théorique avec le modèle continu de Black-Scholes. Le chapitre~3 est consacré à la calibration des paramètres du modèle à partir de données de marché réelles, en détaillant le calcul des rendements logarithmiques, l'estimation de la volatilité historique et le choix du taux sans risque. Le chapitre~4 décrit l'implémentation informatique du projet en Python, organisée en plusieurs modules indépendants couvrant la récupération des données, la calibration et le pricing. Le chapitre~5 présente les expérimentations numériques réalisées sur le CAC~40, avec le pricing de calls et puts européens, d'options américaines, et une analyse de la convergence en fonction du nombre d'étapes. Le chapitre~6 propose un backtest et une validation du modèle par comparaison avec le prix de Black-Scholes sur plusieurs actifs et dans le temps. Enfin, le chapitre~7 discute les limites du modèle et les pistes d'amélioration envisageables.
...
Dans ce chapitre, nous présentons le modèle binomial de Cox, Ross et Rubinstein, souvent appelé modèle CRR. Ce modèle repose sur une représentation discrète de l'évolution du prix du sous-jacent~\cite{cox1979}. À chaque période, le prix du sous-jacent peut uniquement augmenter ou diminuer. Cette structure permet de construire un arbre de prix, puis de calculer progressivement la valeur de l'option.
...
\section{Lien avec le modèle de Black-Scholes}
\label{sec:lien-black-scholes}

Le modèle CRR est un modèle discret qui approxime l'évolution du prix du sous-jacent à l'aide d'un arbre binomial construit sur un nombre fini d'étapes. Lorsque le nombre d'étapes augmente, cette approche se rapproche d'un modèle en temps continu. Dans le cas des options européennes, le modèle de Black-Scholes constitue donc une référence naturelle pour étudier la convergence du prix obtenu par le modèle CRR~\cite{black1973,merton1973}.

\begin{definition}{Modèle de Black-Scholes}{black-scholes}
Le modèle de Black-Scholes est un modèle continu de valorisation des options européennes. Il permet de calculer le prix théorique d'une option à partir du prix initial du sous-jacent $S_0$, du prix d'exercice $K$, de la maturité $T$, du taux sans risque $r$ et de la volatilité $\sigma$.
\end{definition}
...
La fonction $\Phi$ désigne la fonction de répartition de la loi normale centrée réduite.
...
précédents, appelle une vérification numérique à deux niveaux. Le
premier est une validation sur le \textbf{sous-jacent}: la dynamique
discrète de l'arbre binomial est-elle cohérente avec les prix
historiques observés ? Le second est une validation du \textbf{prix
de l'option}: le prix produit par le modèle CRR est-il suffisamment
proche d'une valeur de référence ?

Il convient de préciser d'emblée une limite importante de cette étude.
On ne dispose pas de données de prix d'options réellement cotées sur
le marché: les données récupérées via \texttt{yfinance} fournissent
uniquement les prix historiques du sous-jacent. La validation du prix
de l'option repose donc sur une \textbf{comparaison entre modèles} :
le \textbf{prix prédit} désigne le prix calculé par le modèle CRR, et
le \textbf{prix de référence} désigne le prix analytique de
Black-Scholes, introduit à la section~\ref{sec:lien-black-scholes}.
Cette référence est justifiée par le résultat de convergence: lorsque
$N \to \infty$, le prix CRR converge vers le prix de
Black-Scholes~\cite{crr1979}.

Le backtest est conduit sur deux ensembles d'actifs complémentaires.
D'une part, six valeurs du CAC~40 qui vont être TotalEnergies (TTE.PA), LVMH
(MC.PA), BNP~Paribas (BNP.PA), Sanofi (SAN.PA), Schneider Electric
(SU.PA) et égalément Brent (BZ=F) qui vont permettre une comparaison transversale
sur des profils de volatilité variés. D'autre part, une étude
longitudinale sur le CAC~40 (\texttt{\^{}FCHI}) et le Brent permet
d'analyser la stabilité du modèle dans le temps à l'aide d'un backtest
à fenêtre glissante. Les données couvrent la période du
1\textsuperscript{er}~janvier~2022 au 1\textsuperscript{er}~juin~2025.
Les paramètres retenus sont: maturité $T = 1$~an, $N = 100$ étapes,
call européen, $r = 3\,\%$, $K = S_0$ (\textit{at-the-money}).

\section{Données historiques: prix et rendements du sous-jacent}

Avant toute valorisation, on analyse les données brutes afin de
caractériser le comportement des actifs sur la période étudiée.
La figure~\ref{fig:prix_sousjacents} représente l'évolution des prix
de clôture du CAC~40 et du Brent, et la
figure~\ref{fig:rendements_log} les rendements logarithmiques
journaliers correspondants.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/prix_sousjacents.png}
\caption{Évolution historique des prix de clôture du CAC~40 et du
Brent sur la période 2022--2025.}
\label{fig:prix_sousjacents}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/rendements_log.png}
\caption{Rendements logarithmiques journaliers du CAC~40 et du Brent
sur la période 2022--2025.}
\label{fig:rendements_log}
\end{figure}

On observe que le Brent présente une volatilité annualisée plus élevée
et des chocs de grande amplitude plus fréquents que le CAC~40. Ce
dernier est un indice diversifié sur 40 grandes capitalisations, ce
qui lisse naturellement les chocs individuels. Le Brent, en revanche,
dépend de facteurs géopolitiques, des décisions de production de
l'OPEP et de cycles macroéconomiques globaux, qui génèrent des régimes
de volatilité plus instables~\cite{hull2018}.

\section{Comparaison prix réel du sous-jacent et trajectoire simulée}

Avant de comparer les prix d'options, on vérifie que la dynamique
discrète de l'arbre CRR est cohérente avec les prix historiques
observés. Pour cela, on simule une trajectoire du sous-jacent en
appliquant à chaque pas le facteur $u$ ou $d$ selon la probabilité
risque-neutre $p^*$, en partant du premier prix de clôture de la
période.

Cette comparaison ne cherche pas à reproduire le chemin réel observé :
la trajectoire simulée est \textbf{une réalisation possible parmi une
infinité}. Ce qui importe est que la \textbf{dispersion} et
l'\textbf{ordre de grandeur} des mouvements soient cohérents avec les
données historiques. C'est précisément ce que garantit la calibration
de $\sigma$: les facteurs $u = e^{\sigma\sqrt{\Delta t}}$ et
$d = e^{-\sigma\sqrt{\Delta t}}$ sont construits pour que la variance
des log-rendements du modèle soit égale à $\sigma^2\Delta t$.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/trajectoire_simulee.png}
\caption{Comparaison entre le prix réel observé et une trajectoire
simulée par le modèle CRR, pour le CAC~40 (gauche) et le Brent
(droite). Les deux trajectoires évoluent dans des plages de prix
comparables, ce qui confirme la cohérence de la calibration.}
\label{fig:trajectoire_simulee}
\end{figure}

\section{Comparaison prix prédit / prix de référence}
\label{sec:comparaison_prix}

Pour chaque actif, la volatilité historique annualisée $\sigma$ est
calibrée sur les log-rendements de la période (voir chapitre~3), puis
utilisée à la fois dans le modèle CRR et dans la formule de
Black-Scholes rappelée à la section~\ref{sec:lien-black-scholes} :
\[
C_{\mathrm{BS}} = S_0\,\Phi(d_1) - K\,e^{-rT}\,\Phi(d_2),
\quad
...
Le tableau~\ref{tab:comparaison_crr_bs} présente les résultats obtenus
sur les six actifs.
...
pour chaque actif, le prix CRR à $N=100$ étapes reproduit fidèlement
le prix de Black-Scholes. Les erreurs relatives se situent toutes
autour de $0{,}18\,\%$, ce qui est tout à fait satisfaisant d'un point
de vue pratique~\cite{hull2018}.

La figure~\ref{fig:validation_crr_bs} illustre visuellement ces
résultats sur trois dimensions: comparaison des prix bruts, erreur
relative par actif, et volatilité calibrée.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/validation_crr_bs.png}
\caption{Comparaison CRR / Black-Scholes sur six actifs. (a)~Prix CRR
et Black-Scholes côte à côte. (b)~Erreur relative par actif, avec la
moyenne en pointillés. (c)~Volatilité historique annualisée.}
\label{fig:validation_crr_bs}
\end{figure}

La figure~\ref{fig:comparaison_cac_brent} complète cette analyse en
confrontant directement le CAC~40 et le Brent sur les mêmes dimensions.
Malgré des volatilités différentes, les deux actifs présentent une
erreur relative comparable, ce qui confirme que l'écart CRR / BS est
une propriété de la discrétisation et non de l'actif.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{img/comparaison_cac_brent.png}
\caption{Comparaison CAC~40 / Brent: volatilité annualisée, prix
CRR vs Black-Scholes, et erreur relative.}
\label{fig:comparaison_cac_brent}
\end{figure}

\section{Métriques globales d'erreur}

\begin{definition}{Erreur absolue moyenne (MAE)}{mae}
Soient $\hat{V}_i$ le prix prédit par CRR et $V_i^{\mathrm{BS}}$ le
prix de référence Black-Scholes pour l'actif $i$, avec
$i = 1, \ldots, n$. L'\textit{erreur absolue moyenne} est :
\[
\mathrm{MAE} = \frac{1}{n}\sum_{i=1}^{n}
\bigl|\hat{V}_i - V_i^{\mathrm{BS}}\bigr|.
\]
...
\begin{definition}{Erreur quadratique moyenne (RMSE)}{rmse}
La \textit{racine de l'erreur quadratique moyenne} est :
\[
\mathrm{RMSE} = \sqrt{\frac{1}{n}\sum_{i=1}^{n}
\bigl(\hat{V}_i - V_i^{\mathrm{BS}}\bigr)^2}.
\]
Contrairement à la MAE, le RMSE pénalise davantage les grandes erreurs
individuelles. Un RMSE proche de la MAE indique que les erreurs sont
homogènes d'un actif à l'autre.
\end{definition}

Sur les six actifs, les métriques obtenues à $N = 100$ sont les
suivantes :
\[
\mathrm{MAE} = 0{,}0545,
\qquad
...
Le RMSE est légèrement supérieur à la MAE, ce qui reflète l'erreur
absolue plus grande de LVMH dont le prix absolu est bien plus élevé
que les autres ($S_0 \approx 568$\,€). Les erreurs \textit{relatives}
restent en revanche remarquablement uniformes autour de $0{,}18\,\%$,
ce qui confirme que l'écart est lié à la discrétisation et non à
l'actif. Le tableau~\ref{tab:metriques_globales} résume l'ensemble
des métriques.

\begin{table}[H]
\centering
\caption{Métriques globales d'erreur - CRR vs Black-Scholes
(six actifs, $N=100$).}
...
Métrique & Valeur \\
\midrule
MAE  (erreur absolue moyenne)         & $0{,}0548$ \\
RMSE (racine de l'erreur quadratique) & $0{,}0648$ \\
Erreur moyenne (biais)                & $-0{,}0548$ \\
Erreur absolue maximale               & $0{,}1685$ \\
Erreur relative moyenne               & $0{,}1831\,\%$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Évolution dynamique du prix de l'option - backtest temporel}

Pour aller au-delà d'un calcul ponctuel, on recalibre la volatilité à
chaque date sur une \textbf{fenêtre glissante de 252 jours} (un an de
bourse), puis on calcule le prix théorique de l'option par CRR et par
Black-Scholes. Cette approche permet d'observer comment les prix
évoluent dans le temps en fonction des régimes de volatilité.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/backtest_prix_sigma.png}
\caption{Évolution temporelle du prix théorique de l'option (CRR et
Black-Scholes) et de la volatilité calibrée sur fenêtre glissante de
252 jours, pour le CAC~40 (colonne gauche) et le Brent (colonne
droite).}
\label{fig:backtest_prix_sigma}
\end{figure}

Plusieurs observations ressortent de la
figure~\ref{fig:backtest_prix_sigma}. D'abord, les courbes CRR et
Black-Scholes sont visuellement superposées à toutes les dates,
confirmant la stabilité de l'approximation à $N=100$ dans le temps.
Ensuite, le prix de l'option suit fidèlement les variations de la
volatilité: les périodes de hausse de $\sigma$ se traduisent
directement par une hausse du prix de l'option, conformément à la
théorie. Enfin, la volatilité du Brent est plus instable que celle
du CAC~40, avec des pics plus prononcés, ce qui génère une plus
grande variabilité du prix de l'option dans le temps.

\section{Étude de convergence en fonction de $N$}

Pour illustrer la convergence théorique de façon quantitative, on
fait varier $N$ de 5 à 1\,000 sur l'ensemble des actifs.

\begin{table}[H]
\centering
\caption{Convergence du prix CRR vers Black-Scholes en fonction de
$N$ (TotalEnergies, call ATM, $T=1$~an, $r=3\,\%$,
$\sigma \approx 0{,}253$).}
...
\caption{Convergence du prix CRR vers Black-Scholes en fonction de
$N$ pour le CAC~40 et le Brent. Ligne du haut: prix CRR et
référence BS. Ligne du bas: erreur relative correspondante.}
\label{fig:convergence_crr}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{img/convergence_comparaison.png}
\caption{Comparaison des courbes de convergence entre le CAC~40 et
le Brent. Malgré des volatilités différentes, les deux actifs
présentent une vitesse de convergence similaire.}
\label{fig:convergence_comparaison}
\end{figure}

L'erreur relative décroît rapidement: elle passe d'environ $1\,\%$
pour $N = 5$ à moins de $0{,}02\,\%$ pour $N = 500$, quel que soit
l'actif. La convergence n'est pas strictement monotone au vu du prix CRR qui
oscille autour de la limite Black-Scholes avec des oscillations
alternées selon la parité de $N$~\cite{hull2018}, mais celles-ci
s'amortissent rapidement et deviennent négligeables dès $N \geq 50$.

\section{Analyse des résultats et limites}

\subsection{Bilan de la validation}

Les résultats sont cohérents à tous les niveaux. La dynamique discrète
de l'arbre CRR produit des trajectoires dont l'amplitude est conforme
aux données historiques. Le prix de l'option à $N = 100$ s'écarte de
moins de $0{,}2\,\%$ du prix Black-Scholes sur l'ensemble des actifs
testés, et cette précision est stable dans le temps comme entre
actifs. Le biais moyen légèrement négatif ($-0{,}0548$) traduit les
oscillations résiduelles du modèle discret, phénomène bien documenté
dans la littérature~\cite{hull2018}.

\subsection{Limites}

Deux limites principales encadrent cette validation. En premier lieu,
l'absence de prix d'options cotées empêche toute comparaison avec le
marché réel. Le backtest mesure la \textbf{cohérence interne} entre
deux modèles théoriques partageant les mêmes hypothèses (volatilité
constante, log-normalité, absence de dividendes), et non l'adéquation
du modèle aux prix effectivement pratiqués sur les marchés d'options.

En second lieu, le Brent présente des comportements que ni CRR ni
Black-Scholes ne capturent pleinement: sauts de prix liés à des
événements géopolitiques, queues de distribution plus épaisses qu'une
loi normale, et effets de roulement de contrats à terme. Ces
phénomènes constituent des pistes d'amélioration naturelles, discutées
au chapitre~7.
...
Le modèle de Cox-Ross-Rubinstein repose sur plusieurs hypothèses simplificatrices
qui constituent autant de limites à sa portée pratique.

La première et la plus importante est l'hypothèse de volatilité constante.
Dans notre implémentation, $\sigma$ est estimée une fois sur l'ensemble de la
période historique et reste fixe pour toute la durée de valorisation. Or les
données du CAC 40 montrent clairement que la volatilité varie dans le temps :
elle a dépassé 60~\% annualisé lors du choc Covid de mars 2020, avant de
redescendre à des niveaux bien inférieurs en 2021. Utiliser une volatilité
historique calculée en période calme pour valoriser une option en période de
crise conduit à sous-estimer le prix, et inversement. Ce phénomène, appelé
\textit{volatility clustering}, est une limite structurelle de tous les modèles
à volatilité constante.

Le taux sans risque est lui aussi supposé constant, fixé à $r = 2\%$ par an
pour l'ensemble de la période. En pratique, ce taux dépend de la maturité
considérée et évolue avec les conditions de marché. Sur la période 2018--2026,
les taux ont connu des mouvements significatifs, notamment lors de la remontée
rapide des taux directeurs à partir de 2022. Cette simplification introduit
une erreur d'autant plus grande que la maturité de l'option est longue.

La discrétisation du temps est une autre source d'approximation. L'arbre
binomial remplace la dynamique continue du sous-jacent par une succession de
pas discrets. Bien que le prix CRR converge vers le prix de Black-Scholes
lorsque $N \to +\infty$, pour un nombre fini d'étapes il subsiste une erreur
de discrétisation. Cette erreur est cependant bien contrôlée dès $N = 100$,
comme le montre l'analyse de convergence.

Au-delà des hypothèses du modèle lui-même, des limites apparaissent également
du côté des données. Les rendements logarithmiques du CAC 40 présentent des
queues de distribution plus épaisses que celles d'une loi normale, ainsi qu'une
asymétrie négative: les chocs baissiers extrêmes sont plus fréquents que ce
que le modèle prédit. De plus, l'absence de prix d'options observés sur le
marché empêche toute validation empirique directe du prix théorique.

Sur le plan de la calibration, la volatilité historique est un estimateur
imparfait de la volatilité future, qui est la grandeur réellement pertinente
pour la valorisation d'une option. L'intervalle de confiance calculé à 95~\%
montre que cette estimation est entachée d'une incertitude non négligeable,
qui se propage directement sur le prix théorique.

Plusieurs pistes permettraient d'améliorer le modèle. La plus naturelle serait
d'utiliser une \textit{volatilité implicite} plutôt qu'historique: extraite
des prix d'options observés sur le marché, elle reflète les anticipations des
opérateurs et donne généralement une meilleure estimation du prix. Une
comparaison systématique avec le modèle de Black-Scholes permettrait également
de quantifier l'erreur de discrétisation pour différentes valeurs de $N$ et
de valider la convergence numériquement. Enfin, l'accès à des données réelles
de prix d'options cotées rendrait possible une évaluation empirique rigoureuse
du modèle, en comparant les prix théoriques aux prix de marché observés.
...
Ce rapport a présenté une étude complète du modèle de Cox-Ross-Rubinstein, depuis ses fondements théoriques jusqu'à sa validation empirique sur des données de marché réelles.

Sur le plan théorique, nous avons établi le cadre mathématique nécessaire à la valorisation des options financières : définition du payoff, distinction entre options européennes et américaines, construction de l'arbre binomial, probabilité risque-neutre et actualisation des valeurs futures. Ce cadre, bien que reposant sur des hypothèses simplificatrices, offre une représentation cohérente et interprétable de la dynamique du prix d'un actif sous-jacent.

Sur le plan de la calibration, nous avons montré comment extraire les paramètres du modèle à partir de données historiques : estimation de la volatilité annualisée par l'écart-type des rendements logarithmiques, calcul des facteurs de hausse et de baisse, et détermination de la probabilité risque-neutre. Cette étape est essentielle, car la qualité du prix obtenu dépend directement de la qualité de la calibration.

Sur le plan numérique, les expérimentations menées sur le CAC~40 ont confirmé la cohérence du modèle. Les prix des calls et puts européens satisfont la parité call-put, le put américain vaut bien plus que son équivalent européen, et le prix CRR converge rapidement vers le prix de Black-Scholes à mesure que le nombre d'étapes $N$ augmente. Dès $N = 100$, l'erreur relative est inférieure à $0{,}2\,\%$, ce qui est tout à fait satisfaisant pour un usage pratique.

Sur le plan de la validation, le backtest conduit sur six actifs de profils variés selon CAC~40 et Brent ce qui confirme la stabilité et la robustesse de cette approximation. Les erreurs relatives, uniformément proches de $0{,}18\,\%$, montrent que l'écart entre CRR et Black-Scholes est une propriété de la discrétisation et non de l'actif considéré. L'étude longitudinale par fenêtre glissante révèle en outre que le prix de l'option suit fidèlement l'évolution de la volatilité calibrée dans le temps.

Ces résultats soulignent l'intérêt pédagogique et pratique du modèle CRR comme outil de valorisation simple, progressif et interprétable. Cependant, ses hypothèses de volatilité constante, taux sans risque constant, absence de dividendes et de coûts de transaction en limitent la portée dans des contextes de marché complexes.

Plusieurs pistes permettraient d'aller plus loin. L'utilisation d'une volatilité implicite, extraite des prix d'options cotées, remplacerait avantageusement la volatilité historique et refléterait mieux les anticipations du marché. L'accès à des données réelles de prix d'options permettrait une validation empirique directe, au-delà de la comparaison entre modèles théoriques. Enfin, des extensions du modèle telles que les arbres trinomiaux, les modèles à volatilité stochastique ou les modèles à sauts qui constitueraient des prolongements naturels pour mieux capturer la complexité des marchés financiers réels.
